  ...
  methods: {
    ...
    async login() {
      ...
      if (data.status === 'success') {
        this.page = 'select';  // 로그인 후 페이지를 'select'로 변경
        this.error = '';
      } else {
    ...
    async selectLevel(level) {
      let fileName = '/static/level' + (level === 'easy' ? '1' : level === 'medium' ? '2' : '3') + '_quiz_full_50.json';
      const res = await fetch(fileName);
      const data = await res.json();
      this.currentQuestionSet = data.map(q => ({
        question: q.question,
        options: q.choices,
